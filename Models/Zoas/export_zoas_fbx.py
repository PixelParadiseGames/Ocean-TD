"""
Export Zoas for Roblox — one FBX per size with Main + Accent aligned.
...
Studio layout after import:
    ReplicatedStorage.Coral.Zoas
      ZoaSmall   (Model — MeshParts Main, Accent)
      ZoaMed
      ZoaLarge
"""
...
ROOT = r"C:\Game Dev\Ocean TD\Models\Zoas"
...
def _meshes_in(col_name: str, kind: str) -> list:
    ...
    for obj in col.objects:
        ...
        if kind == "main" and "Main" in obj.name:
            out.append(obj)
        elif kind == "accent" and "Accent" in obj.name:
            out.append(obj)
    if not out:
...
def export_all():
    ...
    print("Done — import each FBX once under ReplicatedStorage.Coral.Zoas")
