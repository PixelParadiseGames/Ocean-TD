...
| Remotes | Live | `src/ReplicatedStorage/OceanTD/Remotes.lua` | `RemoteEvents` folder (not named Remotes — avoids ModuleScript name clash) | `PlotAssigned`, `PlotCleared`, `SessionReady`, `RequestPlace` (RF) |
| GridMath | Live | `src/ReplicatedStorage/OceanTD/Shared/GridMath.lua` | `Constants` | Cell size 4; plot-local helpers |
| InventoryUI | Live | `src/StarterPlayerScripts/OceanTD/InventoryUI.client.lua` | `InventoryState`, `PlacementController`, ItemCatalog, UiCircles | Slot4 backpack; drag-scroll; drag-out place |
...
| PlacementService | Live | `.../Services/PlacementService.lua` | GridService, PlotService, CoralVisual | Server place; hydrate `OceanTD_Placed`; no debit yet |
...
| Remove coral | Planned | PlacementService | GridService | Client request + server clear cell |
